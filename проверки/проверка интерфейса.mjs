...
window.__дай = {
  ...
  кнопкаВФото: (а, и) => кнопкаВФото(а, и),
};
</script>`;
...
console.log("\n10. «Сохранить в Фото»");
// Телефон умеет отдавать файлы в «Поделиться» — как iPhone.
let поделились = null;
окно.navigator.canShare = (что) => !!(что && что.files && что.files.length);
окно.navigator.share = async (что) => { поделились = что; };
// Ролик отдаётся кусками, как настоящий.
окно.fetch = async (адрес) => {
  if (!/raw\.githubusercontent/.test(String(адрес))) {
    return { ok: false, status: 404, json: async () => ({}) };
  }
  const кусок = new Uint8Array(1024);
  let отдан = false;
  return {
    ok: true, status: 200,
    headers: { get: (и) => (и === "content-length" ? "1024" : null) },
    body: { getReader: () => ({
      read: async () => (отдан ? { done: true }
                                : ((отдан = true), { done: false, value: кусок })),
    }) },
    blob: async () => new окно.Blob([кусок], { type: "video/mp4" }),
  };
};

const кнопка = окно.__дай.кнопкаВФото(
  "https://raw.githubusercontent.com/gallxxxx/quiz/video/123.mp4", "мой ролик.mp4");
так("кнопка рисуется, когда копия есть", !!кнопка);
так("и подписана понятно", кнопка && /Сохранить в Фото/.test(кнопка.textContent),
    кнопка && кнопка.textContent);
так("без копии кнопки нет вовсе",
    окно.__дай.кнопкаВФото("", "ролик.mp4") === null);

окно.document.body.appendChild(кнопка);
кнопка.dispatchEvent(new окно.Event("click", { bubbles: true }));
await ждать(150);
так("ролик забран и отдан в «Поделиться»", !!поделились && !!поделились.files,
    JSON.stringify(поделились && Object.keys(поделились)));
так("отдан именно видеофайл",
    поделились && поделились.files[0] && поделились.files[0].type === "video/mp4",
    поделились && поделились.files[0] && поделились.files[0].type);
так("имя файла своё",
    поделились && поделились.files[0].name === "мой ролик.mp4",
    поделились && поделились.files[0].name);
так("ошибок нет", окно.__жалобы.length === 0, JSON.stringify(окно.__жалобы));

// Браузер без «Поделиться файлами» — кнопки быть не должно.
окно.navigator.canShare = undefined;
так("не умеет — не рисуем",
    окно.__дай.кнопкаВФото("https://raw.githubusercontent.com/x/y/video/1.mp4",
                           "р.mp4") === null);
